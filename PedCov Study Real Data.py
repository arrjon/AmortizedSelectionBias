...
from PedCov.helper_functions import plot_delay_distribution, plot_incubation_distribution, plot_generation_time_distribution, normalize_household_data, sampling_parameter_cis, sampling_parameter_cis_variants
#%%
...
if len(variants) == 1:
    adapter = (
        ...
        # todo: check if better: add inference conditions to summary variables
...
else:  # variant also as inference condition
    adapter_inference_condition = (
        bf.adapters.Adapter()
        .drop(['selection_procedure', 'variant'])  # drop strings
        .to_array()
        .one_hot('selection_procedure_id', num_classes=3)  # must be before convert_dtype
...
if len(variants) == 1:
    ...
    model_id_list = itertools.product([0], [0, 1], [8, 18, 24, 36], [18, 24, 36])
else:
    model_id_list = itertools.product([0, 1], [0, 1], [8, 16, 24, 32, 40], [16, 24, 32, 40])
    raise NotImplementedError('not trained yet')
nrmse_list = []
calibration_list = []
model_list = []
sum_i = 0  # transformer summary
for net_id, (adapter_i, inf_i, in_summary_dim, out_summary_dim) in enumerate(model_id_list):
    adapter_name = ''
    if len(variants) == 2:
        adapter = adapter_inference_condition if adapter_i == 0 else adapter_summary_variables
        adapter_name = 'inf_con_' if adapter_i == 0 else 'sum_var_'
    ...
    model_name = (f'{scenario_name}_{adapter_name}{["transformer", "deep_set"][sum_i]}_{["coupling_flow", "flow_matching", "consistency_model"][inf_i]}'
...
for variant in variants:
    ...
    stan_posterior_samples = get_stan_posterior(dfs[variant], param_names, simulator_bf, show_progress=True,
                                                use_simple_model=False)  # simple model is only for testing
