...
def simulator(log_params: np.ndarray,
    ...
    :param non_batchable_context: variant of the simulation (alpha or omicron)
    :param minimal_length: minimal length of the data
    :return: simulated data as numpy array
    """
    selection_procedure, param_alpha = batchable_context
    variant = non_batchable_context
    ...
    fixed_parameters = FIXED_PARAMETERS[variant]
    ...
    for name in PARAM_NAMES:
        if name in fixed_parameters:
            ...
            par_dict.update({name: float(param_alpha)})
